...
function XLSF(oTarget, urlBase) {
    ...
    function ExplosionFragment(nType, sClass, x, y, vX, vY) {
        ...
        this.burst = function () {
            self.oA = new Y.A(self.o, {
                marginLeft: {to: (self.vX * 8)},
                marginTop: {to: (self.vY * 8)}
            }, animDuration, Y.UE.easeOutStrong);
    ...
    function Explosion(nType, sClass, x, y) {
        ...
        this.trigger = function (boxVX, boxVY) {
            ...
            if (!isIE && (IS_MOON_COMPUTER)) {
                var oAExplode = new Y.A(self.o, {
                    marginLeft: {to: 100 * self.boxVX},
                    marginTop: {to: 150 * self.boxVY},
                    opacity: {to: 0.01}
                }, animDuration, Y.UE.easeInStrong);
            } else {
